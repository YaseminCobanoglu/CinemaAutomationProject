...
namespace Project.MVCAdmin.Controllers
{
    public class ReservationController : Controller
    {
      ReservationRepository _reservationRep;
        ...
        [HttpPost]
        public ActionResult AddReservation(AddUpdateReservationPageVM model)
        {
            if (ModelState.IsValid)
            {
                ...
                Reservation r = new Reservation
                {
                    ...
                    TotalPrice = model.Reservation.TotalPrice,
                    /* FilmID = model.Reservation.FilmID,*/ // Film ve Seans ID'lerini eklemeyi unutmayın
                    SeansID = model.Reservation.SeansID,
            ...
            // Eğer ModelState geçerli değilse, tekrar sayfayı göster
            return View(model);
        }
        ...
        {
            // İlgili rezervasyonu veritabanından bulun
            Reservation reservationToDelete = _reservationRep.Find(id);

            // Rezervasyon bulunamazsa, uygun bir hata işleme veya mesaj gösterme kodunu ekleyin.
            if (reservationToDelete == null)
            {
                return HttpNotFound(); // Örnek bir hata durumu
            }

            // Rezervasyonu silin
            _reservationRep.Delete(reservationToDelete);

            // "Index" sayfasına yönlendirme yaparak, rezervasyonun başarıyla silindiği sayfaya gidileceğini belirtiyoruz.
            return RedirectToAction("Index");
        }
